...
class ProductImage implements IModel
{
    public $id;
    public $image;
    public $createdAt;
    ...
    public function __construct($pdo, $data = null)
    {
        ...
        $this->_modelName = 'ProductImage';
        $this->_fieldsOnly = ['id', 'image', 'createdAt', 'updatedAt', 'productId'];
        $this->_fieldsRelated = ['product'];
    ...
    protected function includeProduct(array $items, array $selectedFields = [], array $includeSelectedFields = [], bool $format = false) 
    {
        ...
        $tableName = $dbType == 'pgsql' ? "\"ProductImage\"" : "`ProductImage`";
        $tableModelName = 'ProductImage';
        $tablePrimaryKey = 'id';
    ...
    {
        if (!isset($data['data'])) {
            throw new \Exception("The 'data' key is required when creating a new ProductImage.");
        }
        ...
        foreach ($requiredFieldsMap as $fieldName => $fieldType) {
            if (!isset($data['data'][$fieldName])) {
                throw new \Exception("The '$fieldName' field of type '$fieldType' is required when creating a new ProductImage.");
            }
        }
        ...
        $bindings = [];
        $dbType = $this->_dbType;
        $quotedTableName = $dbType == 'pgsql' ? "\"ProductImage\"" : "`ProductImage`";
                ...
                foreach ($data as $relationName => $relationDataName) {
                    ...
                    if (in_array($relationName, $relationNames)) {
                        ...
                        if (isset($relationDataName['create']) || isset($relationDataName['createMany']) || isset($relationDataName['connect']) || isset($relationDataName['connectOrCreate'])) {
                            throw new \Exception("The relation name '$relationName' is not defined in the ProductImage model.");
                        }
                    }
                }
            }
    ...
    public function findUnique(array $criteria, bool $format = false): array | object
    {
        ...
        if (empty($conditions)) {
            throw new \Exception("No valid 'where' conditions provided for finding a unique record in ProductImage.");
        }
        ...
        foreach ($includes as $relation => $relationInclude) {
            if ($relationInclude) {
                ...
                if (method_exists($this, $includeMethodName)) {
                    ...
                    throw new \Exception("The '$relation' does not exist, in the ProductImage model.");
                }
            }
        }
    ...
    {
        if (!isset($data['where'])) {
            throw new \Exception("The 'where' key is required in the update ProductImage.");
        }
        ...
        if (!isset($data['data'])) {
            throw new \Exception("The 'data' key is required in the update ProductImage.");
        }
        ...
        foreach ($requiredFieldsMap as $fieldName => $fieldType) {
            if (isset($data['data'][$fieldName]) && empty($data['data'][$fieldName])) {
                throw new \Exception("The '$fieldName' field of type '$fieldType' is required in ProductImage Model.");
            }
        }
                    ...
                    foreach ($where as $fieldName => $fieldValue) {
                        ...
                        if (in_array(strtoupper($fieldName), ['AND', 'OR', 'NOT'])) {
                            if (is_array($fieldValue)) {
                                ...
                                foreach ($fieldValue as $subField => $subValue) {
                                    if (array_key_exists($subField, $this->_fields)) {
                                        ...
                                        throw new \Exception("The '$subField' field does not exist in the ProductImage model.");
                                    }
                                }
                            ...
                            if (array_key_exists($fieldName, $this->_fields)) {
                                ...
                                throw new \Exception("The '$fieldName' field does not exist in the ProductImage model.");
                            }
                        }
                    }
                ...
                foreach ($data as $relationName => $relationDataName) {
                    ...
                    if (in_array($relationName, $relationNames)) {
                        ...
                        if (isset($relationDataName['create']) && is_array($relationDataName['create'])) {
                            ...
                            if ($checkArrayContentType !== ArrayType::Value) {
                                throw new \Exception("To create a new record, the value of 'create' must be a single array with names as keys and the data to create as values in product model. example: ['create' => ['name' => 'someName']]");
                            }
                            ...
                            if ($checkArrayContentType !== ArrayType::Associative) {
                                throw new \Exception("To create many records, use an associative array with the field names as keys and the data to create as values in product model. ['createMany' => [['name' => 'someName'], ['name' => 'someOtherName']]");
                            }
                    ...
                    if (isset($relationDataName['connect']) || isset($relationDataName['connectOrCreate']) || isset($relationDataName['disconnect']) || isset($relationDataName['update']) || isset($relationDataName['updateMany'])) {
                        ...
                        if (isset($relationDataName['connect'])) {
                            ...
                            if (isset($relationDataName['connect']) && $checkArrayContentType !== ArrayType::Value) {
                                throw new \Exception("The 'connect' key must be an associative array with the field names as keys and the data to create as values related product model. example: ['connect' => ['id' => 'someId']]");
                            }
                        }
                        ...
                        if (isset($relationDataName['connectOrCreate'])) {
                            ...
                            if (isset($relationDataName['connectOrCreate']) && $checkArrayContentType !== ArrayType::Associative) {
                                throw new \Exception("The 'connectOrCreate' key must be an associative array with the field names as keys and the data to create as values related product model. example: ['connectOrCreate' => ['where' => ['id' => 'someId'], 'create' => ['name' => 'someName']]");
                            }
                        }
                        ...
                        if (isset($relationDataName['disconnect'])) {
                            ...
                            if (!is_bool($disconnectData)) {
                                ...
                                if (isset($relationDataName['disconnect']) && $checkArrayContentType !== ArrayType::Value) {
                                    throw new \Exception("The 'disconnect' key must be an associative array with the field names as keys and the data to create as values related product model. example: ['disconnect' => ['id' => 'someId']]");
                                }
                            }
                        }
    ...
    public function aggregate(array $operation, bool $format = false): array | object
    {
        ...
        $quotedTableName = $this->_dbType == 'pgsql' ? "\"ProductImage\"" : "`ProductImage`";
        $conditions = [];
        $bindings = [];
    ...
    /**
     * Groups records in the 'ProductImage' table and performs aggregate operations.
     * 
    ...
    {
        if (!isset($data['where'])) {
            throw new \Exception("The 'where' key is required in the updateMany ProductImage.");
        }
        ...
        if (!isset($data['data'])) {
            throw new \Exception("The 'data' key is required in the updateMany ProductImage.");
        }
