...
class Product implements IModel
{
    ...
    protected function includeProductImage(array $items, array $selectedFields = [], array $includeSelectedFields = [], bool $format = false) 
    {
        ...
        $instanceModelName = new ProductImage($this->_pdo);
        foreach ($items as &$item) {
            if (!isset($item[$primaryKey])) {
                $item[$relationName] = [];
                continue;
    ...
    protected function includeCartItem(array $items, array $selectedFields = [], array $includeSelectedFields = [], bool $format = false) 
    {
        ...
        $instanceModelName = new CartItem($this->_pdo);
        foreach ($items as &$item) {
            if (!isset($item[$primaryKey])) {
                $item[$relationName] = [];
                continue;
                            ...
                            if (is_array($fieldValue)) {
                                ...
                                foreach ($fieldValue as $subField => $subValue) {
                                    if (array_key_exists($subField, $this->_fields)) {
                                        ...
                                        throw new \Exception("The '$subField' field does not exist in the Product model.");
                                    }
                                }
